...
class PyramidAttention(nn.Module):
    def __init__(self, scale=[1,0.9,0.8,0.7,0.6], res_scale=1, channel=64, reduction=2, ksize=3, \
                stride=1, softmax_scale=10, average=True, ks_padding=1, conv=default_conv):
    ...
    def forward(self, input):
        ...
        for idx, xi in enumerate(input_groups):
            ...
            yi = F.conv_transpose2d(yi, raw_wi, stride=self.stride,padding=self.ks_padding)/4.
            y.append(yi)
...
# used for VGG discriminator
# assuming input is 128x128
class PyramidAttention128(nn.Module):
    def __init__(self, scale=[1,0.9,0.8,0.7,0.6], res_scale=1, channel=64, ksize=3, \
                stride=1, softmax_scale=10, average=True, conv=default_conv):
        
        super(PyramidAttention128, self).__init__()
        self.ksize = ksize
        self.stride = stride
        self.res_scale = res_scale
        self.softmax_scale = softmax_scale
        ...
        escape_NaN = torch.FloatTensor([1e-4])
        self.register_buffer('escape_NaN', escape_NaN)
        self.conv_match_L_base = BasicBlock(conv, channel, channel, 1, bn=False, act=nn.PReLU())
        self.conv_match = BasicBlock(conv, channel, channel, 1, bn=False, act=nn.PReLU())
        self.conv_assembly = BasicBlock(conv, channel, channel, 1, bn=False, act=nn.PReLU())
    ...
    def forward(self, input):
        ...
        input = torch.cat(
        [input[:, :, :32, :32], input[:, :, :32, 32:64], input[:, :, :32, 64:96], input[:, :, :32, 96:],
        input[:, :, 32:64, :32], input[:, :, 32:64, 32:64], input[:, :, 32:64, 64:96], input[:, :, 32:64, 96:],
        input[:, :, 64:96, :32], input[:, :, 64:96, 32:64], input[:, :, 64:96, 64:96], input[:, :, 64:96, 96:],
        input[:, :, 96:, :32], input[:, :, 96:, 32:64], input[:, :, 96:, 64:96], input[:, :, 96:, 96:]
        ], dim=1)
        
        #theta
        match_base = self.conv_match_L_base(input)
        
        shape_base = list(input.size())
        ...
        y = torch.cat(y, dim=0)
        y = torch.split(y, 4, dim=1)

        y_out = torch.empty(res.size()).to('cuda:0')
        y_out[:, :, :32, :32] = y[0]
        y_out[:, :, :32, 32:64] = y[1]
        y_out[:, :, :32, 64:96] = y[2]
        y_out[:, :, :32, 96:] = y[3]
        ...
        y_out[:, :, 32:64, :32] = y[4]
        y_out[:, :, 32:64, 32:64] = y[5]
        y_out[:, :, 32:64, 64:96] = y[6]
        y_out[:, :, 32:64, 96:] = y[7]

        y_out[:, :, 64:96, :32] = y[8]
        y_out[:, :, 64:96, 32:64] = y[9]
        y_out[:, :, 64:96, 64:96] = y[10]
        y_out[:, :, 64:96, 96:] = y[11]
        ...
        return y_out + res*self.res_scale
